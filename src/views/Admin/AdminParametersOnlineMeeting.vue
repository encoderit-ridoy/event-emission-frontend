<template>
	<div class="user-calculation">
		...
		<v-dialog
			...
			scrollable
		>
			<online-meeting-params-form @cancel="dialogCreateNewParam = false" />
		</v-dialog>
		<!-- edit parameters -->
		<v-dialog v-model="dialogEditParam" width="700" persistent scrollable>
			<online-meeting-params-form
...
export default {
	...
	methods: {
		...mapActions(['fetchOnlineMeetingParams', 'deleteOnlineMeetingParams']),
		...
		confirmDeleteItem() {
			this.deleteOnlineMeetingParams(this.deleteItem)
			this.deleteItem = {}
	...
	async mounted() {
		await this.fetchOnlineMeetingParams()
