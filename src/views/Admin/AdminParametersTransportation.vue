<template>
	<div class="user-calculation">
		...
		<v-dialog
			...
			scrollable
		>
			<transportation-params-form @cancel="dialogCreateNewParam = false" />
		</v-dialog>
		<!-- edit parameters -->
		<v-dialog v-model="dialogEditParam" width="700" persistent scrollable>
			<transportation-params-form
...
export default {
	...
	methods: {
		...
		confirmDeleteItem() {
			this.deleteTransportationParams(this.deleteItem)
			this.deleteItem = {}
	...
	async mounted() {
		await this.fetchTransportationParams()
